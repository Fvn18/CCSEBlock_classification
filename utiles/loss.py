...
class SymmetricCrossEntropy(torch.nn.Module):
    def __init__(self, num_classes, alpha, beta, ce_only_epochs=8):
    ...
    def forward(self, pred, labels, epoch=None):
...
def sce(num_classes, config):
    return SymmetricCrossEntropy(num_classes, 
                                config.get('sce_alpha', 0.1), 
                                config.get('sce_beta', 1.0),
                                config.get('sce_ce_only_epochs', 8))

def gce(num_classes, config):
    return GeneralizedCrossEntropy(num_classes, config.get('q', 0.7))
